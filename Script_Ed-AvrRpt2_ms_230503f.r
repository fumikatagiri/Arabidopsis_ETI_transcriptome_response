...
### mock pch, 0,2,5hpt: 0, 1, 5
### Ed pch, 0,1,2,3,4,5,6 hpt: 7,12,10,13,8,9,11
...
#### (mock data, 0, 2, 5 hpt to modeled for the entire time)
...
matplot(0:6, pc1.geno.time, type='n', 
        xlab='Time (hpt)', ylab='PC1 value', las=1)
rect(par("usr")[1],par("usr")[3],par("usr")[2],par("usr")[4],col = "gray")
...
matplot(0:6, AvrRpt2.exp.Ed, type='l', col=rainbow(18, end=0.65), lty=rep(c(1,2,6), 6),
        xlab='Time (hpt)', ylab=expression('AvrRpt2 mRNA level (log'[2]*')'),
...
#text(-0.52, 0.2, 'ETI-PC1, 4hpt', col='gray75', pos=4, srt=90)
#text(-0.71, 0.2, 'ETI-PC1, 6hpt', col='gray65', pos=4, srt=90)
...
matplot(0:6, xve.exp.Ed, type='l', col=rainbow(18, end=0.65), lty=rep(c(1,2,6), 6),
        xlab='Time (hpt)', ylab=expression('XVE mRNA level (log'[2]*')'),
...
#### correlation between XVE level at 0hpt and the AvrRpt2 induction rate from 1 to 4 hpt
...
#### make an equivalent plot with the PC1, 4hpt
### genotype names
...
mp = barplot(rev(quant.comp.nram.pc1[3,]), col=rev(col.b), horiz = T,
        ...
        main='NRAM on ETI-PC1 of ETI-upregulated genes at 4 hpt')
box()
...
mp = barplot(rev(quant.comp.nram.pc1[3,]), col=rev(col.b), horiz = T,
             xlim=range(quant.comp.nram.pc1)*1.04, las=1,
             xlab=expression('Contribution to PC1 (log'[2]*')'),
             main='NRAM on ETI-PC1 of ETI-downregulated genes at 4 hpt')
box()
arrows(quant.comp.nram.pc1[1,],rev(mp), quant.comp.nram.pc1[2,], rev(mp), 
       angle=90, code=3, length=0.075)
dev.off()
...
## R2R1 down strongly associated with P;S at 0hpt > 0
...
venn.diagram(list('R2R1_down' = r2r1.up.genes, 
                  'P;S,0hpt_positive' = basal.p.s.up.genes),
  ...
  for (j in 1:3) {
    for (i in 0:3) {
      ...
      plot(0,0, type='n', 
           ...
           xlab = 'Time (hpt)', ylab='Relative mRNA level (log2)')
