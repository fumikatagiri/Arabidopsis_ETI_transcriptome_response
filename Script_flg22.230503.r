...
load('flg22_raw_gene_counts.RData')
...
png(file = 'flg22.all.samp.x.genes.all.not.scaled2.png', width = 15*900, height = 6.5*600, res=300)
...
save(glm.coefs, genotypes, fix.ef, gene.names, cval.90, ps.c,
     file='flg22.glm.nb.220314.RData')
...
save(genes.dyn,genes.var.geno, file='flg22.selected.genes.dynamics.and.geno.variation.RData')
#######
...
save(all.est, hclust.genes.dyn, file='flg22.hclust.genes.dyn.all.samp.pearson.complete.RData')
data_reordered_full = ams.1[rev(ord),]
...
png(file = 'flg22.all.samp.x.genes.dyn.scaled2.png', width = 15*300, height = 6.5*300, res=300)
print(alloc_vis) 
dev.off()
...
#### load workspace
rm(list=ls())
load('flg22.glm.nb.220314.RData')
...
pdf('eight.FLS2.signaling.genes.pdf', height = 10, width = 7.5)
...
rm(list=ls())
load('flg22.glm.nb.220314.RData')
load('flg22.selected.genes.dynamics.and.geno.variation.RData')
...
date()
save(G.corrected.tcv, file='flg22.corrected.fls2.220314.RData')
...
rm(list=ls())
#### load data
load('flg22.corrected.fls2.220314.RData')
load('flg22.hclust.genes.dyn.all.samp.pearson.complete.RData')
...
png(file = 'flg22.vsf.x.genes.dyn.scaled2.v220315.png', width = 15*300, height = 6.5*300, res=300)
print(alloc_vis) 
dev.off() #15 sec
...
png(file = 'flg22.vsf.x.genes.dyn.nonscaled.v220315.png', width = 15*300, height = 6.5*300, res=300)
print(alloc_vis) 
dev.off()
...
save(flg22.dyn.dat, file='flg22.dyn.dat.v220315.RData')
...
save(flg22.dyn.dat, hclust.flg22.dyn, file='hclust.flg22.dyn.pearson.complete.RData')
...
png(file = 'flg22.vsf.dyn.nonscaled.v220315.png', width = 15*300, height = 6.5*300, res=300)
print(alloc_vis) 
dev.off()
...
#### load workspaces
load('flg22.glm.nb.220314.RData')
load('flg22.dyn.dat.v220315.RData')
load('flg22.corrected.fls2.220314.RData')
...
pdf('SuppFig.forFig5.data.gammamodel.pdf', height = 10, width = 7.5)
opar=par(mfrow=c(6,4), mar=c(2,2,0.5,0.5))
for (gene in genes.flg22.dyn) {
  if (is.na(ud.v[gene])) next
...
save(gm.fit, file='gm.fit.flg22.220320.RData')
...
save(ave.m.out.all, file='flg22.ave.m.out.all.220321.RData')
...
jpeg("PTI.heatmap.E.v230327.jpeg",height = 185, width = 200, units = "mm",res = 300)
...
load('gm.fit.flg22.220320.RData')
...
save(ph.nram, pt.nram, l2a.nram, file='gammamodel.para.NRAM.flg22.v230327.RData')
...
### load data
rm(list=ls())
load('flg22.dyn.dat.v220315.RData') # corrected fls2 subtracted
load('gm.fit.flg22.220320.RData')
load('gammamodel.para.NRAM.flg22.v230327.RData')
...
jpeg("Fig5.PTI.ph.pt.heatmap.v230203.jpeg",height = 160, width = 200, units = "mm",res = 300)
...
pdf('PTI.ph.nram.av.v230327.pdf')
...
pdf('Fig5C.PTI.pt.nram.av.v230327.pdf')
...
pdf('Fig5D.PCA.16geno.res.tc.numb.v230327.pdf', width=10, height = 7.1)
