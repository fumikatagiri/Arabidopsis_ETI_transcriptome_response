...
save(gm.fit, ud.rgu, ud.rgd, ud.v, file='./outputs/gm.fit.flg22.220320.RData')

### find the overlap with ETI-responsive genes
load('./outputs/ETI.gfa.sorted.genes.v230327.RData')

pti.genes = c(ud.rgu, ud.rgd)
eti.genes = c(gfa.names1.posi, gfa.names1.nega)
common.genes = eti.genes[eti.genes %in% pti.genes]
pti.only.genes = pti.genes[!pti.genes %in% eti.genes]
length(common.genes); length(pti.only.genes)
#[1] 1196 common genes, ordered according to the ETI genes, Fig 3A
#[1] 1288 PTI only genes, splitting into up and down, ordered according to pt in JEPS
...
for (gene in c(common.genes, pti.only.genes)) {
...
#### by heatmap
### model values at 0:6 hpt
tx1 = c(0, 2, 3, 5, 9, 18)
model.v.0.18 = t(sapply(gm.fit[c(common.genes, pti.only.genes)], function(x) {
  para.v = x$para.vals
  m.v = c()
  for (genot1 in rownames(para.v)) {
    para.geno = para.v[genot1,]
...
### data
data.v.0.18 = t(sapply(G.corrected.tcv[c(common.genes, pti.only.genes)], function(x) t(x$flg22.vsf[,-2])))
### discrepancy
...
ht_4x = Heatmap(ams.1, col = col, cluster_rows = F, cluster_columns = FALSE,
                ...
                row_title_gp = gpar(fontsize = 10, rot=90),
                ...
                height = unit(125, 'mm'),
                row_gap = unit(1.5, 'mm'),
                row_split = c(rep('Common with ETI', length(common.genes)), rep('PTI only', length(pti.only.genes))),
                )

jpeg("./outputs/FigS4.model.fit2.v230507.jpeg",height = 160, width = 300, units = "mm",res = 300)
...
ave.m.out.all = lapply(G.corrected.tcv, function(v.out) {
  ...
  ave.m.out = c()
  for (hpt in colnames(m.est.geno.h)) {
    m.est.geno = m.est.geno.h[,hpt]
    m.se.geno = m.se.geno.h[,hpt]
...
grid.text('Time (hpt)', x = unit(29,'mm'),y = unit(10,'mm'),gp=gpar(fontsize=8) )
...
#### Fig 5D make an equivalent plot with the PC1, 5hpt
### genotype names
